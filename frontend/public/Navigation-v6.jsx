...
function Navigation() {
  ...
  const goToPage = (e, path) => { e.preventDefault(); setIsMobileMenuOpen(false); navigate(path); };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden md:flex items-center space-x-6">
              ...
              {/* Dropdown Mission */}
              <div className="relative group">
                <a href="/notre-mission" onClick={(e) => goToPage(e, '/notre-mission')} className="text-orange-600 hover:text-orange-700 font-semibold text-base transition-colors duration-200 flex items-center gap-1 py-4">
                  Mission
                  <ChevronDown className="w-4 h-4 transition-transform duration-200 group-hover:rotate-180" />
                </a>
                <div className="absolute top-full left-1/2 -translate-x-1/2 w-72 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform group-hover:translate-y-0 translate-y-2">
                  <div className="bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden">
                    ...
                    <div className="p-2">
                      ...
                      <a href="/vision-travail" onClick={(e) => goToPage(e, '/vision-travail')} className="flex items-center gap-3 px-4 py-3 rounded-lg text-gray-700 hover:bg-amber-50 hover:text-amber-700 transition-all duration-200 group/item">
                        ...
                        <div><p className="font-semibold">Vision du travail</p><p className="text-xs text-gray-500">Le travail comme service</p></div>
                      </a>
      ...
      <div className={`fixed inset-0 bg-black/50 z-40 md:hidden transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsMobileMenuOpen(false)}>
        <div className={`fixed top-20 right-0 bottom-0 w-72 bg-white shadow-2xl transform transition-transform duration-300 overflow-y-auto ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`} onClick={(e) => e.stopPropagation()}>
          <div className="flex flex-col p-6 space-y-2">
            ...
            <a href="/vision-travail" onClick={(e) => goToPage(e, '/vision-travail')} className="flex items-center gap-3 p-3 rounded-lg text-gray-700 hover:bg-amber-50 transition-colors">
              <Eye className="w-5 h-5 text-blue-600" /><span className="font-medium">Vision du travail</span>
            </a>
